...
```bash
$ npm run dev
Express server listening on http://0.0.0.0:5000, in development mode
```
...
```bash
curl -X POST http://0.0.0.0:5000/users -i -d "email=test@example.com&password=123456&access_token=MASTER_KEY_HERE"
```
...
```bash
curl -X POST http://0.0.0.0:5000/auth -i -u test@example.com:123456 -d "access_token=MASTER_KEY_HERE"
```
...
```bash
curl -X POST http://0.0.0.0:5000/articles -i -d "title=Awesome Article&content=Yeah Baby&access_token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9"
```
